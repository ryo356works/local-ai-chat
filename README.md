...
### メイン画面（キャラクター画像あり）
![メイン画面（キャラクター画像あり）](screenshots/main-with-character.png)

サイドバーでスレッド管理、キャラクター画像を表示しながら会話できます。

### メイン画面（シンプル表示）
![メイン画面（シンプル表示）](screenshots/main-simple.png)

サイドバーを閉じてシンプルに。画面を広く使えます。

### 新規スレッド作成
![新規スレッド作成](screenshots/new-thread-dialog.png)

スレッド名、グループ、システムプロンプト、キャラクター画像などを設定できます。
...
各スレッドの設定は`threads/[スレッドID]/thread.yaml`に保存されます
これについてはUI上でスレッドの新規作成時かスレッド名を右クリック→設定でも変更できます
また、template_thread.yamlを編集しておくことで新規作成時に編集する手間が省けます
...
アプリを起動するとui_state.yamlのlast_thread_idを見て最後に開いたスレッド毎のthread.yamlに記載のあるアドレスでバックエンドを立ち上げようとします
